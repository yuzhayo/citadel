"""Browser Camoufox dan transport milik Downloader MangaReader.

Satu plugin fitur untuk pyhost: core tidak tahu arti command di sini, dan
plugin ini tidak menyentuh profile Google milik CamoProf. Profile browser
Downloader tinggal di bawah ``%LocalAppData%\\Citadel\\MangaReader\\browser\\
<provider>``.
...
_API_BRIDGE_JS = r"""
(() => {
    const BRIDGE_ID = 'citadel-downloader-bridge';
...
def _contained_target(path, root):
    """Path tulis harus kanonik di dalam root staging milik Downloader."""
...
async def cmd_open(host, msg):
    ...
    profile_identity = msg.get("profile_identity")
    if profile_identity is not None:
        if not isinstance(profile_identity, str) or not re.fullmatch(r"queue-[a-f0-9]{32}", profile_identity):
            raise PyhostError("BAD_PROFILE", "invalid Queue profile identity")
    ...
    if profile_identity is not None:
        pdir = os.path.join(pdir, "queue", profile_identity)
...
async def cmd_close(host, msg):
    """Tutup satu session Downloader; idempoten seperti session.close core."""
    sid = msg.get("session")
