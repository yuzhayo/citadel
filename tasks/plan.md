...
5. **Waiting happens between commands, and start never blocks on navigation.** All four commands return quickly — `start` returns as soon as the listener is armed, with the login-page navigation running as an enrollment-owned background task (protocol v1 is sequential; an awaited goto would queue a mid-navigation `cancel` behind it for up to 45 s). Teardown cancels and awaits the navigation task. A non-browser navigation failure surfaces as terminal state `failed`. The 10-minute enrollment lifetime is enforced by an internal deadline (background task + status-time check), expiring to `Expired` with full disarm.
