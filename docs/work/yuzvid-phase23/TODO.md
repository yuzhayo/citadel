...
- [x] **T2.1** Wire `core.WebMessageReceived` → `ScriptMessageReceived?.Invoke(e.WebMessageAsJson)`
...
- [x] **T2.2** Upgrade payload `AddScriptToExecuteOnDocumentCreatedAsync`: `__yuzvid.post(obj)` + `__yuzvid.onCommand`
...
- [x] **T3.1** Freeze provenance + port `DEFAULT_VIDEO_HOSTS` → `Features/Extraction/VideoLinkExtractor`
...
- [x] **T3.2** Source A: DOM statik `ExecuteScriptAsync("document.documentElement.outerHTML")` + **JSON decode eksplisit** + batas ukuran
      ...
      - Dep: T3.1
- [x] **T3.3** Source B: tap dinamis — perluas `OnAdBlockResourceRequested` catat URI `.mp4`/`.m3u8`/cdn video
      ...
      - Dep: T2.1
- [x] **T3.4** `LinkDrawerView` + `ExtractionFeature` (kontrak `ExtractAsync`) + slot parent + tombol "re-scan"
...
- [x] **T3.5** Pindah logika Extract dari `YuzvidView.xaml.cs` → parent cuma raise `ExtractRequested`
      - Acceptance: parent tipis, guard lolos, perilaku sama.
      - Dep: T3.4 + izin kamu

## Fase 4 — Queue + download
- [x] **T4.1** `QueueEngine` (opsi 4a: HttpClient+stream, reuse `YuzvidProxyPoolAdapter`)
      - Acceptance: 1 link → file jadi.
      - Verify: smoke unduh 1 file.
      - Dep: T3.4
- [x] **T4.2** Bind `QueueTable.ItemsSource` → `ObservableCollection<QueueItem>` + progress + retry
